...
var player = {
    ...
    infchallengeTimes: [600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31, 600*60*24*31],
...
function getGalaxyRequirement() {
    ...
    if ((player.galaxies) >= getGalaxyCostScalingStart() * 2.5) {
        ...
        amount += (player.galaxies - galaxyCostScalingStart * 2.5)*(player.galaxies - galaxyCostScalingStart * 2.5 - 1) * 4
    }
...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        else if (player.galaxies >= getGalaxyCostScalingStart() * 2.5 || player.currentEternityChall === "eterc5") galString += "Further Antimatter Galaxies (";
    ...
    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            ...
            if (player.eternities > 0)   document.getElementById("postinfi60").innerHTML = "You gain more IP based on galaxies<br>(3^(galaxies-95)) <br>Currently: "+formatValue(player.options.notation,getB60Mult(),2,0)+"x <br>Cost: "+formatValue(player.options.notation,1e50,0,0)+" IP"
	          else document.getElementById("postinfi60").innerHTML = "You gain more IP based on galaxies<br>(2.5^(galaxies-95)) <br>Currently: "+formatValue(player.options.notation,getB60Mult(),2,0)+"x <br>Cost: "+formatValue(player.options.notation,1e50,0,0)+" IP"
...
function updateInfCosts() {
    ...
    if (document.getElementById("timestudies").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        ...
        document.getElementById("161desc").textContent = shortenCosts(new Decimal("1e6660"))+"x multiplier on all normal dimensions"
        document.getElementById("162desc").textContent = shortenCosts(1e234)+"x multiplier on all Infinity dimensions"
...
function getB60Mult(){
    ...
    if (g<95) return 1
    if (player.eternities > 0) return Math.pow(3,g-95)
    return Math.pow(2.5,g-95)
}
...
function setAchieveTooltip() {
    ...
    dimensional.setAttribute('ach-tooltip', "Reach " + formatValue(player.options.notation, 1e12, 0, 0) + " of all dimensions except 8th. Reward: Multipler to ND based on log of dimensions.");
...
function eternity(force, auto) {
	...
	tdBoosts: 0,
		...
		if (player.achievements.includes("r105"))document.getElementById("totaltickgained").textContent += " 'Infinite Time' multiplier: "+shorten(player.postC3Reward.pow(0.05));
        updateTickSpeed();
...
function updateTimeShards() {
    ...
    if(ts.log10()>=dilationstart){
	  ts = Decimal.pow(10, Math.pow(ts.log10()/dilationstart, player.dilation.upgrades.includes(9)?0.75*1.05:0.75)*dilationstart)
	  if (player.dilation.active)ts = Decimal.pow(10, Math.pow(ts.log10()/dilationstart, player.dilation.upgrades.includes(9)?0.75*1.05:0.75)*dilationstart)
    }
    ...
    if(ts1.log10()>=dilationstart){
	  ts1 = Decimal.pow(10, Math.pow(ts1.log10()/dilationstart, player.dilation.upgrades.includes(9)?0.75*1.05:0.75)*dilationstart)
	  if (player.dilation.active)ts1 = Decimal.pow(10, Math.pow(ts1.log10()/dilationstart, player.dilation.upgrades.includes(9)?0.75*1.05:0.75)*dilationstart)
    }
        ...
        else document.getElementById("timeShardsPerSec").textContent = "You are getting "+shortenDimensions(getTimeDimensionProduction(1))+" Timeshards per second ("+shortenDimensions(ts1.sub(ts))+" after dilation). Time dimension multipliers and Timeshards are dilated "+(player.dilation.active?"twice ":"")+" after " + shorten(Decimal.pow(10,getDilationStart()));
...
function gameLoop(diff) {
    ...
    if(ts.log10()>=dilationstart){
	  ...
	  if (player.dilation.active)ts = Decimal.pow(10, Math.pow(ts.log10()/dilationstart, player.dilation.upgrades.includes(9)?0.75*1.05:0.75)*dilationstart)
        ...
        player.tickThreshold = new Decimal(1).times(tsMult).pow(player.totalTickGained - Math.pow(player.tickspeedBoosts,0.9)*100)
    ...
    for (var tier = 1; tier < 9; tier++) {
		...
		if(player.eternities<1)document.getElementById("timeMax"+tier).style.display="none";else document.getElementById("timeMax"+tier).style.display="inline-block";
